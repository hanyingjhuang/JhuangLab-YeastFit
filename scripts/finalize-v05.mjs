...
const matrixBody="assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');if(await page.locator('#viz_effects .main-svg').count()===0){const detail=await page.locator('[data-analysis-module=\"metricTests\"]').innerText();const debug=await page.evaluate(()=>window.__YEASTFIT_TEST_DEBUG);throw new Error(`matrix effect figure missing; integrated comparisons:\n${detail}\nDEBUG=${JSON.stringify(debug)}`)}";
const matrixBlock=`if(id==='matrix'){${matrixBody}}`;
const replacements=[
  ["if(id==='matrix')assert.ok(await page.locator('#viz_effects .main-svg').count()===1,'matrix should produce matched-control effect statistics');",matrixBlock],
  ["if(id==='matrix')assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');await page.locator('#viz_effects .main-svg').first().waitFor({state:'attached',timeout:8000});",matrixBlock],
  ["if(id==='matrix')assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');if(await page.locator('#viz_effects .main-svg').count()===0){const detail=await page.locator('[data-analysis-module=\"metricTests\"]').innerText();throw new Error(`matrix effect figure missing; integrated comparisons:\n${detail}`)}",matrixBlock],
  ["if(id==='matrix')assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');if(await page.locator('#viz_effects .main-svg').count()===0){const detail=await page.locator('[data-analysis-module=\"metricTests\"]').innerText();const debug=await page.evaluate(()=>window.__YEASTFIT_TEST_DEBUG);throw new Error(`matrix effect figure missing; integrated comparisons:\n${detail}\nDEBUG=${JSON.stringify(debug)}`)}",matrixBlock],
