...
const replacements=[
  ["assert.ok(await page.locator('#viz_effects .main-svg').count()===1,'matrix should produce matched-control effect statistics');","assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');await page.locator('#viz_effects .main-svg').first().waitFor({state:'attached',timeout:8000});"],
  ...
  ["await page.locator('#viz_effects .main-svg').waitFor({state:'attached',timeout:8000});","await page.locator('#viz_effects .main-svg').first().waitFor({state:'attached',timeout:8000});"],
