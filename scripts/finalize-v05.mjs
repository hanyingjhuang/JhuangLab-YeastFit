...
const matrixDiagnostic="assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');if(await page.locator('#viz_effects .main-svg').count()===0){const detail=await page.locator('[data-analysis-module=\"metricTests\"]').innerText();const debug=await page.evaluate(()=>window.__YEASTFIT_TEST_DEBUG);throw new Error(`matrix effect figure missing; integrated comparisons:\n${detail}\nDEBUG=${JSON.stringify(debug)}`)}";
const replacements=[
  ...
  ["assert.equal(await page.locator('#viz_effects').count(),1,'matrix effect card should exist');if(await page.locator('#viz_effects .main-svg').count()===0){const detail=await page.locator('[data-analysis-module=\"metricTests\"]').innerText();throw new Error(`matrix effect figure missing; integrated comparisons:\n${detail}`)}",matrixDiagnostic],
