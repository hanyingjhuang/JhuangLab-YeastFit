...
function metricGrid(root,rows,group){const metrics=['mu_max','doubling_time','lag','auc','max_value','endpoint'].filter(m=>rows.some(r=>Number.isFinite(+r[m])));for(const m of metrics){const el=card(root,`viz_metric_${m}`,m.replaceAll('_',' '),`Biological-replicate distribution of ${m.replaceAll('_',' ')} across groups.`);const lo=base(m.replaceAll('_',' '));lo.xaxis.title=group;lo.showlegend=false;plot(el,boxTraces(rows,group,m),lo);}}

export function renderVisualDashboard(root,{S,data,group,metric}){
  if(!root)return;root.innerHTML='';
  const preset=S.design.preset||'manual',points=data.inferencePoints||[],metrics=data.inferenceMetrics||S.metrics||[],norm=data.normalizedPoints||[],tests=data.metricTests?.length?data.metricTests:data.timepointTests||[];
  ...
  if(['daily','evolution'].includes(preset)&&group){
    const end=card(grid,'viz_endpoint','Endpoint distribution','Biological-replicate endpoint values, with individual observations.');let lo=base('Endpoint');lo.xaxis.title=group;lo.showlegend=false;plot(end,boxTraces(metrics,group,'endpoint'),lo);
    const auc=card(grid,'viz_auc','Integrated phenotype','AUC across sparse sampled timepoints. This summarizes total exposure to the phenotype, not growth rate.');lo=base('AUC');lo.xaxis.title=group;lo.showlegend=false;plot(auc,boxTraces(metrics,group,'auc'),lo);
  ...
  if(['endpoint','manual'].includes(preset)&&group){
    const e=card(grid,'viz_endpoint_box','Group distributions','Raw endpoint distributions at the biological-replicate level.');let lo=base('Endpoint');lo.xaxis.title=group;lo.showlegend=false;plot(e,boxTraces(metrics,group,metric||'endpoint'),lo);
  ...
  if(preset==='matrix'){
    ...
    if(group){const b=card(grid,'viz_matrix_groups','Group distributions','Biological-replicate phenotype across genotypes or strains.');lo=base(metric||'endpoint');lo.xaxis.title=group;lo.showlegend=false;plot(b,boxTraces(metrics,group,metric||'endpoint'),lo);}
  }
  if(preset==='dose'){
    const dose=(S.factors||[]).find(x=>/dose|concentration|conc/i.test(x));if(dose){const d=card(grid,'viz_dose','Dose-response curves','Median biological-replicate response at each observed concentration.',true);let lo=base('Response');lo.xaxis.title=dose;plot(d,doseTraces(points.length?points:metrics,dose,group),lo);}
